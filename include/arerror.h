...
struct ArpackError_static
{
 ...
 protected:
   ...
   // orenaud / vhuber: removed `code` static variable.
  // The static variable here was removed. It was causing an error when linking
  // an object file that contained this header into a shared library.
...
class ArpackError: public ArpackError_static<void> {
 ...
 private:
  ...
  void Print(const std::string& where, const std::string& message);
  // Writes error messages on cerr stream.

 public:

  void Set(ErrorCode error, const std::string& where="AREigenProblem");
  // Set error code and write error messages.

  int Status() { return (int) code; }
